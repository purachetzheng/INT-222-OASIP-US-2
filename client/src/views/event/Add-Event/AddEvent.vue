...
const addEventForm = reactive({
    ...
    isStepComplete: computed(() => {
        if (addEventForm.step === 1)
            return addEventForm.selectedCategoryId !== 0
        // return true
    ...
    <PageWrapper>
        <div class="min-h-full flex flex-col">
            <header class="flex justify-between items-center">
                <div class="">
                    <h1 class="text-2xl font-semibold">Book Event</h1>
                </div>
            </header>
            <div class="flex gap-8 mt-4">
                <div class="bg-white rounded-lg shadow-lg p-4 h-fit">
                    <TabStep
                        :step-now="addEventForm.step"
                        :step-info="stepInfo"
                ...
                <div class="bg-white rounded-lg shadow-lg p-4">
                    <h1 class="text-xl font-semibold mb-4">
                        {{ stepInfo[addEventForm.step - 1].name }}
                    </h1>
                    <SelectCategory
                    ...
                    />
                    <FillForm
                        v-if="addEventForm.step === 2"
                        :add-event-form="addEventForm"
                        @submit-form="onSubmit"
                    />

                    <div
                        class="flex justify-between flex-row-reverse gap-4 mt-4"
